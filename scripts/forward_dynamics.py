...
with open(dirpath+"/../logs/traj_sawyer_push.txt") as file:
        ...
        for row in reader:
                if line == 0:
                        ...
                        qpos.append(np.array(list(map(float, row[8:15]))))
                        qvel.append(np.array(list(map(float, row[21:28]))))
                        qacc.append(np.array(list(map(float, row[34:41]))))
                        ctrl.append(np.array(list(map(float, row[41:48]))))
                        qfrc.append(np.array(list(map(float, row[54:61]))))
                        qext.append(np.array(list(map(float, row[67:74]))))
                        bias.append(np.array(list(map(float, row[80:87]))))
                        xfrc.append(np.array(list(map(float, row[87:93]))))
                        line += 1
                else:
                        break
